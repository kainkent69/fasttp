...
Map-based, deferred-execution HTTP test framework for [Gin](https://github.com/gin-gonic/gin).
...
func TestHello(t *testing.T) {
    ...
    tr.Data(func(data interface{}) {
        m := data.(map[string]interface{})
        assert.Equal(t, "world", m["message"])
    })
    tr.Run()
}
```
...
func TestHello(t *testing.T) {
    fasttp.NewFunc(t, "hello", "/hello", nil, func(tr *fasttp.T) fasttp.TestInfo {
        return fasttp.TestInfo{
            ...
            JSON:   true,
            Data: func(data interface{}) {
                assert.Equal(t, "world", data.(map[string]interface{})["message"])
            },
        }
    })
}
...
| `YAML()` | `*Body` | Stub — requires `gopkg.in/yaml.v3` dependency. |
...
| `Data(fn func(interface{}))` | `*T` | Append callback receiving parsed response. Multiple allowed. **Shadows `Body.Data()`** — raw bytes accessor stays reachable as `t.Body.Data()`. |
